...
export default function LeastSold() {
  const [data, setData] = useState<LeastSold[]>([]);

  useEffect(() => {
    fetch("http://localhost:3001/api/sales/product/leastsold")
      .then((res) => res.json())
      .then((items) => {
        setData(
          items.map((d: any) => ({
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow">
      <h3 className="font-semibold mb-2">Produk Paling Sedikit Terjual</h3>
