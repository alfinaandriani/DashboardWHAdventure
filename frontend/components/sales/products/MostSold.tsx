...
export default function MostSold() {
  const [data, setData] = useState<MostSold[]>([]);

  useEffect(() => {
    fetch("http://localhost:3001/api/sales/product/mostsold")
      .then((res) => res.json())
      .then((items) => {
        setData(
          items.map((d: any) => ({
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow">
      <h3 className="font-semibold mb-2">Produk Paling Banyak Terjual</h3>
